...
export default function SuccessDialog({ isVisible, dismissFunc }: props) {
  ...
  return (
    <Portal>
      ...
      >
        <Dialog.Icon
          icon={() => <Ionicons name={'checkmark-circle'} style={style.icon} />}
        />
        ...
        <Dialog.Content style={style.content}>
          <PaperText style={style.dialogText} variant="bodyMedium">
            {i18n.t('confirmDialog.checkOutComplete')}
          </PaperText>
...
const styles = (theme: {
  COLORS: { WARNING: string; WHITE: string; PRIMARY: string };
  TYPOGRAPHY: {
    FONT_WEIGHT: any;
